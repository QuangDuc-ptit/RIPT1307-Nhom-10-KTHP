...
interface AuthState {
  ...
  loading: boolean;
  initialized: boolean;
  login: (payload: LoginPayload) => Promise<{ user: User; accessToken: string; refreshToken: string }>;
  socialLogin: (payload: { provider: string; idToken: string }) => Promise<{ user: User; accessToken: string; refreshToken: string }>;

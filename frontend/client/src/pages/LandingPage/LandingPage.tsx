...
import { Row, Col, Typography, Space, Button, Input, Form, Divider, Modal } from 'antd';
import { GlobalOutlined, VideoCameraOutlined, ShareAltOutlined } from '@ant-design/icons';
...
const LandingPage = () => {
  ...
  const footerStyles = { surfaceVariant: '#462f2c', onSurfaceVariant: '#e9bcb6', primary: '#ff1e00', primaryRed: '#E50914' };

  return (
    <>
      <header className="header">
        <div className="logo" onClick={() => scrollToSection("home")}><h2>KSTAR</h2></div>
        <nav className="navbar">
          ...
          <Link to="/profile" className="nav-link-custom" onClick={(e) => handleProtectedAction(e, () => navigate("/profile"))}>Cá nhân</Link>
        </nav>
        <button
          onClick={() => navigate('/auth/login')}
          style={{
            background: 'linear-gradient(135deg, #E50914 0%, #b2070f 100%)', border: 'none', color: 'white', fontWeight: 600, fontSize: '0.95rem',
            padding: '8px 24px', borderRadius: '40px', cursor: 'pointer', transition: 'all 0.25s ease', boxShadow: '0 4px 12px rgba(229, 9, 20, 0.3)', marginLeft: '16px',
          }}
        >
          Đăng nhập
        </button>

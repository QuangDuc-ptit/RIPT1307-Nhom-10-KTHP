...
const Sidebar: React.FC<Props> = ({ movie }) => {
  ...
  // Tự động load lại bình luận khi URL đổi sang phim khác
  useEffect(() => {
    setReviews([
  ...
  // 🚀 HÀM ĐẨY BÌNH LUẬN MỚI LÊN ĐẦU DANH SÁCH
  const handleSubmitReview = () => {
    ...
    const newReview = {
      id: Date.now(),
      author: `${authUser.firstName || ''} ${authUser.lastName || ''}`.trim() || 'Người Dùng',
      avatar: authUser.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${authUser.firstName}`,
      content: reviewText,
      date: new Date().toLocaleDateString('vi-VN') 
    };

    // Nối bình luận mới vào trước mảng cũ
    setReviews([newReview, ...reviews]);
    
    setReviewText('');
    setShowReviewForm(false);
    setShowReviewList(true); // Tự động mở list để khoe bình luận vừa đăng
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ backgroundColor: colors.bgCard, borderRadius: '16px', border: `1px solid ${colors.border}`, padding: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '24px' }}>
          <div style={{ color: colors.primary, fontSize: '48px', fontWeight: '900', lineHeight: 1 }}>{movie.reviews?.average || 0}</div>
          <div>
            <Rate disabled defaultValue={5} style={{ color: colors.primary, fontSize: '14px' }} />
            <div style={{ color: colors.textDim, fontSize: '12px', marginTop: '4px' }}>Dựa trên {movie.reviews?.totalVotes || 0} lượt đánh giá</div>
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px' }}>
          {movie.reviews?.breakdown?.map((item: any) => (
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <Button 
            ...
            style={{ flex: 1, backgroundColor: showReviewForm ? colors.primary : 'transparent', borderColor: colors.primary, color: showReviewForm ? '#fff' : colors.primary, borderRadius: '20px', height: '40px', fontWeight: 'bold' }}
          >
          ...
          <Button 
            ...
            style={{ flex: 1, backgroundColor: showReviewList ? '#2d2025' : 'transparent', borderColor: '#2d2025', color: '#fff', borderRadius: '20px', height: '40px', fontWeight: 'bold' }}
          >
        ...
        {showReviewList && (
          <div style={{ marginTop: '20px', maxHeight: '320px', overflowY: 'auto', paddingRight: '10px', borderTop: `1px solid ${colors.border}`, paddingTop: '16px' }}>
            <List
              dataSource={reviews}
              renderItem={item => (
                <List.Item style={{ borderBottom: `1px solid ${colors.border}`, padding: '12px 0' }}>
                  <List.Item.Meta
                    avatar={<Avatar src={item.avatar} style={{ backgroundColor: '#fcdfd5', color: colors.primary }}>{!item.avatar && item.author.charAt(0)}</Avatar>}
                    title={<div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#fff', fontWeight: 'bold' }}>{item.author}</span><span style={{ color: colors.textDim, fontSize: '11px', fontWeight: 'normal' }}>{item.date}</span></div>}
